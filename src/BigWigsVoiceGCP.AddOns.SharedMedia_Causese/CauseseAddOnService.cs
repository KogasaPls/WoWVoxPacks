    ...
    : IAddOnService
{
    ...
    {
        var fileNameWithoutExtension = Path.GetFileNameWithoutExtension(soundFile.FileName);
        var fileNameWithWavExtension = $"{fileNameWithoutExtension}.wav";

        Logger.LogInformation("Creating sound file {FileName}", soundFile.FileName);
        var audioContent = await SynthesizeText(soundFile, cancellationToken);


        await File.WriteAllBytesAsync(Path.Combine(outputDirectory, fileNameWithWavExtension), audioContent.ToByteArray(), cancellationToken);
